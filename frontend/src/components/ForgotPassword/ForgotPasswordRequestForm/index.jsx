...
const ForgotPasswordRequestForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    console.log("API URL:", "http://localhost:8080/api/applicants/forgot-password");
    
    setLoading(true);
    
    try {
      console.log("Making axios POST request...");
      const response = await axios.post("http://localhost:8080/api/applicants/forgot-password", {
        email: data.email
      }, {
        headers: {
      ...
      showSnackbar(response.data.message || "Reset link sent to your email.", "success");
    } catch (error) {
