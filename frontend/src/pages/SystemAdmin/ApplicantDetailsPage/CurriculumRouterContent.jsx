...
export default function CurriculumRouterContent({ curriculumId, curriculum: propCurriculum, loading: propLoading, inModal = false }) {
  const theme = useTheme();
  const [curriculum, setCurriculum] = useState(propCurriculum || null);
  ...
  useEffect(() => {
    if (curriculumId && !propCurriculum) {
      setLoading(true);
  ...
  }, [curriculumId, propCurriculum]);

  // Update local state when prop curriculum changes
  useEffect(() => {
    if (propCurriculum) {
  ...
  // Update local loading state when prop loading changes
  useEffect(() => {
