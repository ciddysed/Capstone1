...
// Create a separate component for editable cells to manage local state
const EditableTextField = ({ value, onSave, placeholder, multiline = false, rows = 1, isGradeField = false }) => {
  ...
  useEffect(() => {
    debouncedSave.current = debounce((newValue) => {
  ...
  const validateGrade = (gradeValue) => {
    ...
    if (numValue < 1.0) {
      return "Grade cannot be below 1.0";
    }
    
    if (numValue > 5.0) {
      return "Grade cannot be above 5.0";
    }
  ...
  const handleChange = (e) => {
    ...
    if (isGradeField) {
      const validationError = validateGrade(newValue);
      setError(validationError);
      
      // Only trigger save if valid
      if (!validationError) {
  ...
  return (
    <TextField
      ...
      error={!!error}
      helperText={error}
      type={isGradeField ? "text" : "text"}
      inputProps={isGradeField ? { 
        inputMode: 'decimal',
        pattern: '[0-9]*\\.?[0-9]*'
      } : {}}
      sx={{
...
const GradedAccreditation = ({ applicantId: propApplicantId, curriculumId: propCurriculumId }) => {
  // Persist props to sessionStorage for page refresh
  const getPersistedValue = (key, propValue) => {
    const storageKey = `gradedAccreditation_${key}`;
    if (propValue !== undefined && propValue !== null) {
      sessionStorage.setItem(storageKey, String(propValue));
      return propValue;
    }
...
<<<<<<< HEAD
    ...
    try {
      ...
      setRecords(prev => ({
        ...
        [semester]: prev[semester].map(r =>
          r.id === recordId ? { ...r, grade: newGrade } : r
        )
      }));
    } catch (err) {
  ...
  const saveProcessOfAccreditation = useCallback(async (recordId, semester, newProcess) => {
    try {
      const params = new URLSearchParams();
      ...
      setRecords(prev => ({
        ...prev,
        [semester]: prev[semester].map(r =>
          r.id === recordId ? { ...r, processOfAccreditation: newProcess } : r
        )
      }));
    } catch (err) {
  ...
  const saveSubstantiveBasis = useCallback(async (recordId, semester, newBasis) => {
    try {
      const params = new URLSearchParams();
      ...
      setRecords(prev => ({
        ...prev,
        [semester]: prev[semester].map(r =>
          r.id === recordId ? { ...r, substantiveBasis: newBasis } : r
        )
      }));
    } catch (err) {
...
<<<<<<< HEAD
                            ...
                            <StyledTableCell>
                              {!isLocked ? (
                                <EditableTextField
                                  value={rec.grade}
                                  onSave={(newGrade) => saveGrade(rec.id, semester, newGrade)}
                                  placeholder="1.0 - 5.0"
                                  isGradeField={true}
                                />
                              ) : (
                            ...
                            <StyledTableCell>
                              {!isLocked ? (
                                <EditableTextField
                                  ...
                                  onSave={(newProcess) => saveProcessOfAccreditation(rec.id, semester, newProcess)}
                            ...
                            <StyledTableCell>
                              {!isLocked ? (
                                <EditableTextField
                                  ...
                                  onSave={(newBasis) => saveSubstantiveBasis(rec.id, semester, newBasis)}
